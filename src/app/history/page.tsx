  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 5 }}>
        ...
        {/* Summary */}
        <div style={{ color: "rgba(248,242,228,0.6)", fontSize: 14 }}>{entry.summary}</div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {entry.playerNames.slice(0, 5).map((name, i) => (
            <span key={i} style={{
              ...
              padding: "2px 8px", color: "rgba(248,242,228,0.7)", fontSize: 13,
            }}>
              {name}
            </span>
          ))}
          {entry.playerNames.length > 5 && (
            <span style={{ color: "rgba(248,242,228,0.35)", fontSize: 13 }}>+{entry.playerNames.length - 5}</span>
          )}
        </div>
        ...
        {entry.winnerLabel && (
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ color: GOLD, fontSize: 15, fontWeight: 700 }}>{entry.winnerLabel}</span>
          </div>
        )}
      </div>

      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3, flexShrink: 0 }}>
        <div style={{ color: "rgba(248,242,228,0.3)", fontSize: 13 }}>{formatDate(entry.createdAt, locale)}</div>
        <div style={{ color: "rgba(248,242,228,0.2)", fontSize: 13 }}>{formatTime(entry.createdAt)}</div>
    ...
    <div style={{ minHeight: "100vh", background: FELT_DK, display: "flex", flexDirection: "column", maxWidth: 390, margin: "0 auto", fontFamily: "system-ui,-apple-system,sans-serif" }}>
      ...
      <div style={{ background: FELT, padding: "44px 16px 16px", display: "flex", alignItems: "center", gap: 12, direction: dir }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: "rgba(248,242,228,0.4)", fontSize: 13, marginTop: 1 }}>
        ...
        {([
          ...
          >
            ...
            {key === "active" && active.length > 0 && (
              <span style={{ marginRight: locale === "ar" ? 4 : 0, marginLeft: locale === "en" ? 4 : 0, background: "#4CAF50", color: "#fff", borderRadius: "50%", padding: "0 5px", fontSize: 13, fontWeight: 700 }}>
                {active.length}
              </span>
            )}
          </button>
      ...
      <div style={{ flex: 1, padding: "16px 16px 40px", display: "flex", flexDirection: "column", gap: 10, direction: dir }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, paddingTop: 60, textAlign: "center" }}>
            ...
            <div style={{ color: "rgba(248,242,228,0.3)", fontSize: 15 }}>
